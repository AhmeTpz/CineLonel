# ===================================================
# AIRING TODAY TV SHOWS PAGE / BUGÜN YAYINLANAN TV DİZİLERİ SAYFASI
# ===================================================
...
async function fetchAllShows() {
  try {
    ...
    while (hasMorePages && currentPage <= 30) {
      const response = await request<{ total_pages: number; results: TvShow[] }>('/tv/airing_today', {
        page: currentPage,
        region: 'US'
      })
...
// Runtime cache for TV (episode_run_time)
  ...
  async function worker() {
    while (idx < toFetch.length) {
      ...
      try {
        ...
        const run = Array.isArray(detail.episode_run_time) && detail.episode_run_time.length > 0 ? detail.episode_run_time[0] : null
        runtimeCache.set(cur.id, run ?? null)
      } catch {
...
// Sayfa özel provider sayımı ve show->provider eşlemesi
...
<template>
  <PageLayout>
    <HeroSection
      title="Airing Today"
      subtitle="TV shows airing today"
    ...
    <ContentSection
      ...
      loading-text="Loading airing today TV shows..."
