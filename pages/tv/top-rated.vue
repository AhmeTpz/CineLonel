# ===================================================
# TOP RATED TV SHOWS PAGE / EN YÜKSEK PUANLI TV DİZİLERİ SAYFASI
# ===================================================
...
async function fetchAllShows() {
  try {
    ...
    while (hasMorePages && currentPage <= 75) {
      const response = await request<{ total_pages: number; results: TvShow[] }>('/tv/top_rated', {
        page: currentPage,
        region: 'US'
      })
  ...
  async function worker() {
    while (idx < toFetch.length) {
      ...
      try {
        ...
        runtimeCache.set(cur.id, run as number | null)
      } catch {
...
<template>
  <PageLayout>
    ...
    <ContentSection
      ...
      loading-text="Loading top rated TV shows..."
