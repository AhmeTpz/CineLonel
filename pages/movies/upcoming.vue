# ===================================================
# UPCOMING MOVIES PAGE / YAKINDA ÇIKACAK FİLMLER SAYFASI
# ===================================================
...
async function fetchAllMovies() {
  try {
    ...
    while (hasMorePages && currentPage <= 20) { // Limit to 20 pages to avoid infinite loop
      const response = await request<{ total_pages: number; results: Movie[] }>('/movie/upcoming', { 
        page: currentPage,
...
<template>
  <PageLayout>
    <HeroSection
      ...
      subtitle="Exciting movies coming to theaters soon"
      gradient-class="bg-gradient-to-r from-purple-600 via-pink-600 to-red-600"
      title-gradient-class="bg-gradient-to-r from-purple-300 to-pink-300 bg-clip-text text-transparent"
      :stats="[
    ...
    <ContentSection
      ...
      loading-text="Loading upcoming movies..."
      loading-color="border-purple-500 border-t-transparent"
      no-results-title="No Movies Found"
      no-results-text="Try adjusting your search or check back later for new releases."
      icon-gradient="bg-gradient-to-r from-purple-500 to-pink-500"
