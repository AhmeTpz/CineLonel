# ===================================================
# POPULAR MOVIES PAGE / POPÜLER FİLMLER SAYFASI
# ===================================================
...
async function fetchMovies(pageNum: number, filters?: FilterOptions) {
  try {
    ...
    if (filters) {
      ...
      if (filters.releaseDateTo) {
        ...
        const maxMovieDate = new Date()
        maxMovieDate.setFullYear(currentDate.getFullYear() + 1)
        
        params['primary_release_date.lte'] = maxMovieDate.toISOString().split('T')[0]
      }
    }

    const response = await request<{ total_pages: number; results: Movie[] }>('/discover/movie', params)

    let movies = response.results || []
    
    if (filters && (filters.minRuntime > 0 || filters.maxRuntime < 360)) {
      const detailedMovies = await Promise.all(
        movies.map(async (movie) => {
          try {
            const detailResponse = await request<{ runtime: number }>(`/movie/${movie.id}`)
            return { ...movie, actualRuntime: detailResponse.runtime }
          } catch {
            return { ...movie, actualRuntime: null }
          }
        })
      )
      
      movies = detailedMovies.filter(movie => {
        if (movie.actualRuntime === null) return true
        return movie.actualRuntime >= filters.minRuntime && movie.actualRuntime <= filters.maxRuntime
      })
    }
...
function handleKeywordsChange(list: { id: number; name: string }[]) {
  currentFilters.value.keywords = list
}

function handleSearch() {
  router.push({ query: {} })
  fetchMovies(1, currentFilters.value)
}

const isSearching = ref(false)

onMounted(async () => {
  fetchMovies(page.value, currentFilters.value)
  
  setTimeout(async () => {
    isProvidersLoading.value = true
    try {
      const cached = getCachedCounts('movie')
      if (Object.keys(cached).length > 0) {
        providerCounts.value = cached
      } else {
        providerCounts.value = await fetchGlobalProviderCounts('movie')
      }
    } catch (error) {
...
watch(() => page.value, (newPage) => {
  fetchMovies(newPage, currentFilters.value)
})
...
<template>
  <PageLayout>
    <HeroSection
      ...
      subtitle="The most popular movies trending right now"
      gradient-class="bg-gradient-to-r from-red-600 via-pink-600 to-purple-600"
      title-gradient-class="bg-gradient-to-r from-yellow-300 to-orange-300 bg-clip-text text-transparent"
      :stats="[
    ...
    <ContentSection
      ...
      loading-color="border-red-500 border-t-transparent"
      no-results-title="No Movies Found"
      no-results-text="Try adjusting your search or check back later for new releases."
      icon-gradient="bg-gradient-to-r from-red-500 to-pink-500"
      @reset-filters="() => { currentFilters = { sortBy: 'popularity', sortOrder: 'desc', genres: [], whereToWatch: [], minRating: 0, maxRating: 10, releaseYear: '', minRuntime: 0, maxRuntime: 360, minVotes: 0, maxVotes: 50000, language: '', releaseDateFrom: '', releaseDateTo: '', keywords: [] }; router.push({ query: {} }); fetchMovies(1, currentFilters); }"
    >
      <template #pagination>
